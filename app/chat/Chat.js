...
import { Animated, Keyboard, Platform, ScrollView, Text, TextInput, TouchableOpacity, View, ActivityIndicator } from "react-native";
...
export default function Chat() {
        ...
        (async () => {
            try {
                const response = await fetch(`http://${Globals.ip}:3080/api/getHangoutById`, {
                    ...
                    body: JSON.stringify({ userId, hangoutId: id }), signal
                });
                if (!response.ok) {
    ...
    useEffect(() => {
        ...
        const newSocket = io(`http://${Globals.ip}:3090`, {
            query: { userId }, autoConnect: true, reconnection: true,
        });
        setSocket(newSocket);
    ...
    useEffect(() => {
        ...
        const hideSub = Keyboard.addListener("keyboardDidHide", () => { setKeyboardHeight(0); scrollViewRef.current?.scrollToEnd({ animated: true }); });
    ...
    const encriptarMensaje = (mensaje) => {
        ...
        cipher.start({ iv }); cipher.update(forge.util.createBuffer(mensaje)); cipher.finish();
        ...
        return { message: forge.util.encode64(encryptedMessage), iv: forge.util.encode64(iv), encryptedAESKey: encryptedKeys };
    };
    ...
    const desencriptarMensaje = (mensaje) => {
        try {
            ...
            for (const ts of Object.keys(mensaje)) {
                ...
                dec.start({ iv }); dec.update(forge.util.createBuffer(forge.util.decode64(mObj.mensaje.message)));
                if (!dec.finish()) continue;
                const texto = dec.output.toString();
                setMessages(prev => [...prev, { usuario: mObj.usuario, idUsuario: mObj.idUser, mensaje: texto, tiempo: mObj.tiempo }]);
            }
        } catch (error) {
    ...
    return (
        <View style={{ paddingTop: Constants.statusBarHeight }} className="bg-[#DBF3EF] w-full min-h-full h-screen relative">
            ...
            <View style={{ flex: 1, marginBottom: textInputHeight }}>
                <ScrollView ref={scrollViewRef} className="px-3 flex flex-col gap-y-5" contentContainerStyle={{ paddingBottom: 10 }} onContentSizeChange={() => scrollViewRef.current?.scrollToEnd({ animated: true })}>
                    {messages.map((msg, i) => (
                        <View key={i} className={`pl-6 pr-2 pt-3 pb-1 rounded-lg max-w-[70%] mt-4 ${msg.idUsuario === userId ? 'bg-blue-500 self-end rounded-br-none' : 'bg-gray-700 self-start rounded-bl-none'}`}>
                            {msg.idUsuario !== userId && <Text className="text-yellow-300 self-start">{msg.usuario}</Text>}
                            <Text className="text-white pr-4">{msg.mensaje}</Text>
                            <Text className="text-sm text-white opacity-60 self-end leading-1 pl-32">{new Date(msg.tiempo).toLocaleTimeString()}</Text>
                        </View>
                    ))}
                </ScrollView>
            </View>
            ...
            <View style={{ bottom: keyboardHeight }} className="absolute bg-white w-full flex gap-x-4 flex-row justify-center p-4">
                <TextInput placeholder="Escribe un mensaje..." multiline value={text} onChangeText={setText} onLayout={handleLayout} blurOnSubmit={false} className="p-2 border border-gray-300 rounded-2xl" style={{ maxHeight: 100, minHeight: 35, marginBottom: 20, width: inputWidth }} />
                {buttonVisible && (
                    <Animated.View style={{ opacity: fadeAnim, justifyContent: 'center', alignItems: 'center' }}>
                        <TouchableOpacity style={{ backgroundColor: '#00ad3a', width: 48, height: 48, borderRadius: 24, justifyContent: 'center', alignItems: 'center' }} onPress={sendMessage}>
